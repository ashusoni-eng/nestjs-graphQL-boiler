...
export const generateNotificationEmailTemplate = (title: string, description: string, startTime: string, endTime: string, address: string,): string => `
  <div style="font-family: sans-serif; border: 1px solid #e0e0e0; border-radius: 8px; padding: 24px; max-width: 600px; margin: 30px auto; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.05);">
    <h2 style="text-align: center; color: #2c3e50; margin-bottom: 20px;">${title}</h2>
    <p style="font-size: 16px; color: #34495e; margin-bottom: 10px;">${description}</p>

    <div style="margin: 20px 0; padding: 16px; background-color: #f9f9f9; border-radius: 4px;">
      <p style="font-size: 15px; margin: 8px 0;"><strong>Start:</strong> ${startTime}</p>
      <p style="font-size: 15px; margin: 8px 0;"><strong>End:</strong> ${endTime}</p>
      <p style="font-size: 15px; margin: 8px 0;"><strong>Location:</strong> ${address}</p>
    </div>
